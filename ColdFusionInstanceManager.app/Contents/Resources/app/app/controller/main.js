...
function createIRow(cItem)
{
        ...
        cf_port.innerHTML = getHTTPPort(cItem);
        status.innerHTML = 
        `   
        <div class="switch center">
            ...
            <div class="preloader-wrapper active center" id="ldr_${cItem}" 
                 style="width:26px;height:26px;float:right;margin-top:-20px;display:none">
        ...
        `
        var portstr = (cf_port.innerHTML != "") ? ":" + cf_port.innerHTML : "";
        ...
        <a href="http://localhost${portstr}/CFIDE/administrator/index.cfm" 
        ...
        if(cItem == "cfusion") $(btnEdit).addClass("disabled");

        //Get Status
        //execute(`ps aux | grep "java -classpath /Applications/ColdFusion11/${cItem}/" | grep -v grep | wc -l`,
        //createCallback(cItem),false);
...
function getHTTPPort(id)
{
    ...
    var cfserverxml = fs.readFileSync(cfserverxml,'utf-8');
    var Elements = $(cfserverxml).find("connector");
    return Elements.last().attr("port");
}
...
function getVHost(id)
{
    ...
    var result = VList.find(function(item)
    {
        ...
        {
            Elements = Elements[0];
            var filename = Elements.replace(/^.*[\\\/]/, '').toLowerCase();
            //Check if it belogns to the instance we are looking.
            if(filename == id.toLowerCase() + ".properties")
                return item.trim();
        }  
    })
...
function isJVMPortValid(id)
{
    ...
    return !PortList.includes(InstancePort);
}

/**
 * Get a list with all Java ports being used by Instances
 * 
 * @param {any} exludeid - Optional, Exlude a Instance.
 * @returns {Array} - Array with all JAVA ports.
 */
function getJVMPortList(exludeid)
{
...
/**
 * Creates a callback that reads correctly the id.
 * 
 * @param {String} _cfid - Instance ID
 * @returns {Function} - Callback Function
 */
function createCallback(_cfid)
{
    return function(output)
    {
        ...
        $("#ldr_"+_cfid).hide();
        //Set Status
        $("#status_"+_cfid).prop("checked",output.trim() == "1");
        //Enable input
        $("#status_"+_cfid).prop("disabled",false);  
        //Remove css effect.
        if(output.trim() == "1") 
            ShowButtons(_cfid);
    }
...
function RefreshStatus(bForce)
{
    ...
    {
        //execute(`ps aux | grep "java -classpath /Applications/ColdFusion11/${cItem}/" | grep -v grep | wc -l`,
        execute(`ps -ef | grep "java -classpath /Applications/ColdFusion11/${cItem}/" | grep -v grep | wc -l`,
        createCallback(cItem),false,cItem,bForce);        
    });    
}
...
function getListIssues(id)
{
    ...
    [
        ...
        FileExist(CFProperties + "/" + id + ".properties",true),    //Properties Status
...
function FixIssues(id,url,FolderPath)
{
    ...
    if(!LArray[1])
    {
        ...
        fs.writeFile(CFProperties + "/" + id + ".properties",NewProperties);
    }
...
function RemoveVHost(id)
{
    ...
    {
        if(item.includes(id+".properties"))
            return item;
            //VHList.splice(index,1);
    });
    //console.log(ItemVhost);
    //Update file.
...
function DeleteReferences(id)
{
    ...
    if(LArray[0])
    {
        ...
        delete config[wIndex + ".max_reuse_connection"];
    ...
    if(LArray[1])
    {
        fs.unlinkSync(CFProperties + `${id}.properties`);
    }
        ...
        function (error, response, body) 
        {
            ...
            if( action == "create" )
            {
                ...
                if(AddServerURL.length <= 0)
                {
                    ...
                    console.log(body);
                    Materialize.toast('Error while creating instance.', 6000,"red");
                    callback(true);
                    return;
                }
...
{
    return getCFIDList().includes(id);
}
    ...
    function(bError)
    {
        ...
        if(!bError)
        {
            if( action == 'create' )
            {
                ...
                Materialize.toast('Instance \''+ id + '\' was created succesfuly', 6000,"green");                
            }
            else
            if( action == 'delete' )
            {
